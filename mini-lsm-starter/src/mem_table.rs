...
impl MemTable {
    ...
    pub fn scan(&self, lower: Bound<&[u8]>, upper: Bound<&[u8]>) -> MemTableIterator {
        let mut iter = MemTableIteratorBuilder {
            map: self.map.clone(),
            iter_builder: |iter_builder: &Arc<SkipMap<Bytes, Bytes>>| {
                iter_builder.range((map_bound(lower), map_bound(upper)))
            },
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        let res = self.with_mut(|fields| match fields.iter.next() {
            Some(entry) => {
